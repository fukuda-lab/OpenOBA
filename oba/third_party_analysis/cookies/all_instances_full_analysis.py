...
def load_csv_and_sqlite_to_df(experiment_name):
    ...
    df_grouped = df_grouped[["session", "host_domain"]]

    # Group by session and leave only unique domains
    df_grouped = df_grouped.groupby("session")["host_domain"].unique().reset_index()

    # Rename host_domain to domains
    df_grouped = df_grouped.rename(columns={"host_domain": "domains"})
...
df_num_domains.to_markdown(
    f"{RESULTS_DIR}/cookie_domains_by_session_all_instances.md", index=False
)
